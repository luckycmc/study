 ...
 * 主线程会将用户的socket连接信息封装成CQ_ITEM，并放入工作线程的处理队列中。
 * 工作线程得到主线程的pipe通知后，就会将队列中的ITEM取出来，创建libevent的socket读事件。
 */
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        //这个方法非常重要，主要是创建每个线程自己的libevent的event_base也就是每一个线程创建一个reactor
